...
async function enviarCorreoBrevo(venta, codigos) {
  ...
  const qrCodes = codigos.map(codigo => {
    const qrUrl = `https://chart.googleapis.com/chart?chs=250x250&cht=qr&chl=${encodeURIComponent(codigo)}&choe=UTF-8`;
    return `
      <div style="text-align:center;margin-bottom:16px;">
        <img src="${qrUrl}" alt="QR ${codigo}" style="width:200px;height:200px;border-radius:10px;border:4px solid #fff;"/>
        <div style="font-family:monospace;font-size:11px;color:rgba(255,255,255,0.4);margin-top:6px;">${codigo}</div>
      </div>`;
  }).join('');
...
<body style="font-family:Arial,sans-serif;background:#1a0f2e;color:#fff;margin:0;padding:0;">
  <div style="max-width:520px;margin:0 auto;padding:32px 20px;">
    ...
    <div style="background:#1c1c28;border-radius:16px;padding:24px;margin-bottom:16px;text-align:center;">
      ...
      <p style="font-size:11px;color:rgba(255,255,255,0.35);margin:0 0 16px;">Muestra este QR en la entrada del evento</p>
      ${qrCodes}
    </div>
    ...
    <p style="text-align:center;color:rgba(255,255,255,0.25);font-size:11px;">McCarthy's Irish Pub · Boca del Río, Ver.<br>© 2025 Amnistía</p>
  </div>
</body>
</html>`;
  ...
  await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      sender: { name: 'Amnistía Tickets', email: 'tickets@amnistia.com' },
